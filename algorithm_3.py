...
class MarkovVolatilityStopLossModel(QCAlgorithm):
    """
    This algorithm is the exact same as algorithm 2. The difference being that the stop loss 
    is set to .95 (allowing <= 5% loss) for low volatility markets and .98 for high volatility ones.
    The idea is to allow for more room for trades to breathe during low volatility regimes,
    and reducing exposure during high volatlity regimes. If the stop loss isn't hit by the end of the week, 
    the algorithm automatically cancels it and liquidates the position. A new order is placed at the beginning
